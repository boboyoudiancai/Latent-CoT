...
def main(cfg) -> None:
    ...
    cfg.framework.cot_mode = "none"
    cfg.framework.cot_mode_flags = mode_flags
    cfg.datasets.vla_data.bridge_reasoning.enable = False
    cfg.datasets.vla_data.bridge_reasoning.stage = 0

    training_stage = getattr(cfg.framework, "training_stage", "full")

    logger.info(f"[Pure QwenGR00T] training_stage={training_stage}, flags={mode_flags}")
