...
from laravla.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps
from laravla.model.modules.action_model.fast_ActionHeader import Fast_Action_Tokenizer
from laravla.dataloader.gr00t_lerobot.bridge_annotations import BridgeAnnotations

from laravla.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from laravla.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from laravla.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        self.bridge_reasoning_as_solution = False
        self.include_action_tokens = False
        if enable_reasoning:
            from laravla.dataloader.gr00t_lerobot.bridge_reasoning_formatter import BridgeReasoningFormatter

            stage = int(reasoning_cfg.get("stage", 0)) if isinstance(reasoning_cfg, dict) else 0
            as_solution = bool(reasoning_cfg.get("as_solution", False)) if isinstance(reasoning_cfg, dict) else False
            ...
            self.bridge_reasoning_as_solution = as_solution
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        try:
            if steps_path.exists():
                ...
                if cached_key is None or cached_key != config_key:
                    ...
                    return cached_data["steps"]
        except (FileNotFoundError, EOFError, pickle.PickleError, KeyError) as e:
        ...
        try:
            ...
            if should_write:
                ...
                tmp_steps_path = steps_path.with_name(
                    f".{steps_path.name}.{os.getpid()}.tmp"
                )
    ...
    def _build_sample_from_data(self, data: dict, trajectory_id: int, base_index: int) -> dict:
        ...
        if self.bridge_reasoning_formatter is not None:
            ...
            formatted = self.bridge_reasoning_formatter.format(
                instruction=original_language,
                sample=sample,
            )
            if self.bridge_reasoning_as_solution:
                sample["cot_solution"] = formatted
                sample["language"] = original_language
                sample["lang"] = original_language
            else:
